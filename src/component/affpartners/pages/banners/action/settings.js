									...
									<Field name="is_shown">
										{({ field, form }) => (
											<Form.Field className={(() => {return form.touched[field.name] && form.errors[field.name] ? 'error' : "";})()}>
												<Checkbox className="labelagree" toggle id="change_pass" label="Show the banners page in your affiliates' panels?" checked={state.is_shown} 
													onChange={(e)=>{
														let ischecked = 'false';
														if(e.target.checked==true){
															ischecked = 'true';
														}
														setstate({...state,is_shown:ischecked});
													}}
												/>
											</Form.Field>
										)}
