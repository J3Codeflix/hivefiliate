...
export default function OrdersApproved(props) {
    ...
    if(usersContext){
        ...
        if(stafflog!=0){
          view_order  		    = usersContext.staff_permission.order.view=='true'?true:false;
          edit_order  		    = usersContext.staff_permission.order.edit=='true'?true:false;
        }
    }
    ...
    function SearchProcess(){
        const searchvalue = {
            ...
            date_from:search.date_from!=''?moment(new Date(search.date_from)).format('YYYY-MM-DD'):'',
            date_to:search.date_to!=''?moment(new Date(search.date_to)).format('YYYY-MM-DD'):'',
            is_order:search.is_order,
        }
		...
		<React.Fragment>
	        <div className="orders pagecontent">
                    ...
                    {successmsg&&<AlertSuccess CloseTheAlert={CloseAlert} sizeWidth='full' TextAlert={alertmessage}/>}
                        ...
                        <Menu icon='labeled'>
                            <Menu.Item as='a' onClick={()=>Public_URL('/orders/approved')} active><Icon name='calendar check outline'/>Approved Orders</Menu.Item>
                            <Menu.Item as='a' onClick={()=>Public_URL('/orders/pending')}><Icon name='calendar times outline'/>Pending Orders</Menu.Item>
                            <Menu.Item as='a' onClick={()=>Public_URL('/orders/denied')}><Icon name='calendar times outline'/>Denied Orders</Menu.Item>
                        </Menu>
                    </div>
