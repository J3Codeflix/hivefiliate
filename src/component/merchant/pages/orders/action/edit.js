...
export default function Updateorder(props) {
   ...
   /* Modal close */
	...
	function showAlert(data){
		...
		props.textalertMessage('Order successfully updated');
	}
   ...
   function handleSubmitForm(values, { props = this.props, setSubmitting, resetForm, setErrors }){
		...
		formData.append('type','merchant_updateorder');
   ...
   function getDetails(){
        let formData = new FormData();
        formData.append('type','merchant_getorder');
        formData.append('id',props.idController);
        axios.post('/merchant/orders/request.php',formData)
        .then(function (response) {
            let obj = response.data;
            let dateback = moment(new Date(obj.date_order)).format('MM/DD/YYYY');
            setstate({
                ...state,
                id:props.idController,
                affiliate_id:obj.affiliate_id,
                order_id:obj.order_id,
                tracking_method:obj.tracking_method,
                order_price:obj.order_price,
                aff_earnings:obj.aff_earnings,
                date_order:new Date(dateback),
                order_status:obj.order_status,
                landing_page:obj.landing_page,
                referal_page:obj.referal_page,
                notes:obj.notes,
            ...
            setdate_dummy(new Date(dateback));
						getaffiliateinfo(obj.affiliate_id);
        })
        .catch(function (error) {return false;});
    }
	   ...
	   const [affpercent, setaffpercent] = useState(0);
	 	 function getaffiliateinfo(id){
	 				 let formData = new FormData();
	 				 formData.append('type','merchant_affinfo');
              ...
              enableReinitialize
	            ...
	            const { values, isSubmitting, errors, handleChange, handleBlur, handleSubmit, isValid, touched, setFieldValue } = formProps
		          return(
					        ...
					        <Modal.Header>Update order <span className="floatright" onClick={()=>closeModal(false)}><i className="ti-close"></i></span></Modal.Header>
                     ...
                     <div className="columns is-mobile">
                        ...
                        <div className="columns is-mobile">
                            <div className="column is-one-quarter"><label className="inlinelabel text-blur">Order ID</label></div>
                            <div className="column">
                                <Form.Group widths='equal'>
                                    <Field name="order_id">
                                        {({ field, form }) => (
                                        <Form.Field className={(() => {return form.touched[field.name] && form.errors[field.name] ? 'error' : "";})()}>
                                            <Input fluid {...field} onChange={handleChange} autoComplete={isoff} readOnly={true}/>
                                            { form.touched[field.name] && form.errors[field.name] && <Label className="ui above prompt label errorlabel"><i class="fa fa-exclamation-triangle"></i>{form.errors[field.name]}</Label> }
                                        </Form.Field>
                                        )}
                                    </Field>
                                ...
                                <Field name="order_price">
                                    ...
                                    <Form.Field className={(() => {return form.touched[field.name] && form.errors[field.name] ? 'error' : "";})()}>
                                        <Input iconPosition='left'>
                                            <Icon name='dollar' />
																							<Cleave {...field} disabled={values.is_paid==1} options={{numeral: true,numeralThousandsGroupStyle: 'thousand'}}
																								onChange={(e)=>{
                            ...
                            <div className="column">
                                ...
                                <Field name="aff_earnings">
                                    ...
                                    <Form.Field className={(() => {return form.touched[field.name] && form.errors[field.name] ? 'error' : "";})()}>
                                        <Input iconPosition='left'>
                                            <Icon name='dollar' />
                                            <Cleave disabled={values.is_paid==1} {...field} options={{numeral: true,numeralThousandsGroupStyle: 'thousand'}} onChange={handleChange} autoComplete={isoff}/>
                                            <Button
																						    disabled={values.is_paid==1}
                                                color='green'
                                                icon='calculator'
                                                labelPosition='left'
                                                content="Calculate"
                                ...
                                <Field name="referal_page">
                                    ...
                                    <Form.Field className={(() => {return form.touched[field.name] && form.errors[field.name] ? 'error' : "";})()}>
                                        ...
                                        { form.touched[field.name] && form.errors[field.name] && <Label className="ui above prompt label errorlabel"><i class="fa fa-exclamation-triangle"></i>{form.errors[field.name]}</Label> }
												...
												<div className="columns is-mobile">
                            <div className="column is-one-quarter"><label className="inlinelabel text-blur">Location</label></div>
                            <div className="column">
                                <Form.Group widths='equal'>
                                <Field name="location_type">
                                    {({ field, form }) => (
                                    <Form.Field>
                                        <Input fluid {...field} onChange={handleChange} readOnly={true}/>
                                    </Form.Field>
                                    )}
                                </Field>
                                </Form.Group>
												...
												<div className="columns is-mobile">
                            <div className="column is-one-quarter"><label className="inlinelabel text-blur">IP Address</label></div>
                            <div className="column">
                                <Form.Group widths='equal'>
                                <Field name="ip_address">
                                    ...
                                    <Form.Field>
                                        <Input fluid {...field} onChange={handleChange} readOnly={true}/>
												...
												<div className="columns is-mobile">
                            <div className="column is-one-quarter"><label className="inlinelabel text-blur">Device Type</label></div>
                            <div className="column">
                                <Form.Group widths='equal'>
                                <Field name="device_type">
                                    ...
                                    <Form.Field>
                                        <Input fluid {...field} onChange={handleChange} readOnly={true}/>
												...
												<div className="columns is-mobile">
                            <div className="column is-one-quarter"><label className="inlinelabel text-blur">Order From</label></div>
                            <div className="column">
                                <Form.Group widths='equal'>
