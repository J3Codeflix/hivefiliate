...
export default function PendingAffiliates(props) {
	...
	if(usersContext){
			...
			if(stafflog!=0){
				view_aff  		  = usersContext.staff_permission.affiliate.view;
				view_edit  		  = usersContext.staff_permission.affiliate.edit;
				view_pay  		  = usersContext.staff_permission.affiliate.pay;
				view_delete  		= usersContext.staff_permission.affiliate.delete;
			}
	}
                ...
                <div className="table-action">
                    <div className="table-buttons">
                        <div className="columns is-mobile is-vcentered">
                            <div className="column"><h2 className="titlewrapper">Pending Affiliates</h2></div>
                            <div className="column">
															  {view_edit==true&&<div className="position-right">
																	...
																	<Button className='black' icon><Icon name='bullseye' /> Sent Payment</Button>
															</div>}
                            </div>
                        </div>
                    </div>
                    <div className="table-navigation">
                        <Menu icon='labeled'>
                            <Menu.Item as='a' href={process.env.PUBLIC_URL+'/mrcaffiliates/active'}><Icon name='user outline'/>Active Affiliates</Menu.Item>
                            <Menu.Item active as='a' href={process.env.PUBLIC_URL+'/mrcaffiliates/pending'}><Icon name='user outline'/>Pending Applications</Menu.Item>
                            <Menu.Item as='a' href={process.env.PUBLIC_URL+'/mrcaffiliates/denied'}><Icon name='user outline'/>Denied Applications</Menu.Item>
                            <Menu.Item as='a' href={process.env.PUBLIC_URL+'/mrcaffiliates/block'}><Icon name='user outline'/>Block Affiliates</Menu.Item>
                            <Menu.Item as='a' href={process.env.PUBLIC_URL+'/mrcaffiliates/deleted'}><Icon name='user outline'/>Deleted Affiliates</Menu.Item>
                        </Menu>
                    </div>
                </div>
                        ...
                        <Form>
                            <div className="columns is-mobile is-vcentered">
                                ...
                                <div className="column is-one-fifth">
                                    <Form.Group widths='equal'>
                                        <Button fluid className='blue' icon onClick={()=>SearchProcess()}><Icon name='search' /> Search</Button>
                                    </Form.Group>
                                </div>
                            </div>
                        </Form>
				    	...
				    	<div className="columns is-mobile is-vcentered">
			              ...
			              <div className="column">
                              <div className="position-right">
                                <Button className='black' icon onClick={()=>showLegend(true)}><Icon name='tasks' /> Legend</Button>
                            ...
                            <Table.Body>
                                {list.length==0&&<Table.Row negative><Table.Cell  colSpan='9' textAlign="center">No record found</Table.Cell></Table.Row>}
			                    		  {list.map(function(data, key){
	  					          						return <Table.Row key={key} warning>
																			...
																			<Table.Cell>{data.pending_orders}</Table.Cell>
																			<Table.Cell positive>{data.paid_earnings}</Table.Cell>
																			<Table.Cell>{data.unpaid_earnings}</Table.Cell>
