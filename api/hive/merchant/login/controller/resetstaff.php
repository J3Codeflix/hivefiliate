...
class Controller extends Database{
  ...
  function Checkemail($email){
    $query="select * from staff WHERE email=:email and status='Active'";
  ...
  function InfoName($email){
    $query="select first_name,last_name from staff WHERE email=:email and status='Active'";
    $stmt = $this->conn->prepare( $query );
    $stmt->bindValue(':email',$email);
    $stmt->execute();
  ...
  function CheckResetID($id){
    $query="select * from staff WHERE reset_key=:reset_key and is_reset=1 and status='Active'";
    $stmt = $this->conn->prepare( $query );
  ...
  function staffemail($id){
    $query="select * from staff WHERE id=:id and status='Active'";
    $stmt = $this->conn->prepare( $query );
  ...
  function checkExpired($id){
    $query="select * from staff WHERE reset_key=:reset_key and is_reset=1 and status='Active' and reset_date='".date('Y-m-d')."'";
    $stmt = $this->conn->prepare( $query );
    $stmt->bindValue(':reset_key',$id);
    $stmt->execute();
    return $stmt->rowCount();
