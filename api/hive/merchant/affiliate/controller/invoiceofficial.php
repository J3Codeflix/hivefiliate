...
class Controller extends Database{
  ...
  function isnotpaid($month,$year){
    $query ="select * from orders
    WHERE merchant_id='".$this->session."' and (YEAR(date_order)='".$year."' and MONTH(date_order)='".$month."') and is_paid=0";
    $stmt = $this->conn->prepare($query);
    $stmt->execute();
    return $stmt->rowCount();
  }
    ...
    $array = array();
	    foreach ($months as $row) {
	      $array[] = [
           ...
           'isnotpaid'    	  => $this->isnotpaid($row['id'],$row['year']),
