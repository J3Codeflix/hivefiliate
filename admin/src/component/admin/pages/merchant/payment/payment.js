...
export default function MerchantInvoicePayment(props) {
													...
													{list.map(function(data, key){
														return <Table.Row key={key} className={data.is_process==1?'positive':'negative'}>
															  ...
															  <Table.Cell>{data.month_invoice}</Table.Cell>
																...
																<Button className='teal' size='tiny' icon as="a" href={LinkURL('/merchant/paymentapproval/?id='+data.reference_id)}><Icon name='check circle' /> Payment Approval</Button>
