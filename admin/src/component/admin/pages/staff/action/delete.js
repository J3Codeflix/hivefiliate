...
export default function DeleteAccount(props) {
  ...
  function handleSubmitForm(values, { props = this.props, setSubmitting, resetForm, setErrors }){
		...
		formData.append('type','admin_staffdelete');
      ...
      setspinner(false);
			if(obj==1){
				resetForm();
				if(buttonclick==1){callalert(true,{text:'Staff successfully deleted on the database',type:'success',size:'full',open:true});reloadlist();closeModal(false);}
		...
		<div className="modalwrapper">
		     <Formik
	            ...
	            const { values, isSubmitting, errors, handleChange, handleBlur, handleSubmit, isValid, touched, setFieldValue } = formProps
		          return(
				        ...
				        <Modal.Header>Delete Confirmation<span className="floatright" onClick={()=>closeModal(false)}><i className="ti-close"></i></span></Modal.Header>
					      <Modal.Content className="modalcontent colormodalred">
                {isucess&&Success()}
                {iserror&&Error()}
                ...
                <Message
                  ...
                  header='Delete Staff'
                  list={[
                    'This action is not ireversible.',
                    'The account and their associated information will be deleted on the database'
                  ]}
                />
